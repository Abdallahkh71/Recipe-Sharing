    ...
    (function () {
        ...
        if (!token) {
            Swal.fire({
                ...
                customClass: {
                    confirmButton: 'btn btn-outline-danger' // Apply Bootstrap classes here
                }
            }).then(() => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!token) {
        ...
        document.querySelector('#user-login').textContent = 'LOGIN';

        // document.querySelector('#user-login a').setAttribute('href', 'login&signup.html') = 'LOGOUT'
    } else {
        document.querySelector('#useron span').textContent = tokenData.name;
        document.querySelector('#user-login').textContent = 'LOGOUT';

        // document.querySelector('#user-login a').setAttribute('href','index.html') = 'LOGOUT'
    }
...
function updateRecipe(index) {
    ...
    // Set the selected option for dishType and difficulty
    Array.from(dishType.options).forEach(option => {
        option.selected = option.text === recipe.dishType;
    });

    Array.from(difficulty.options).forEach(option => {
        option.selected = option.text === recipe.difficulty;
    });

    // Set image preview if an image was uploaded
    if (recipe.image) {
        const imagePreview = document.querySelector('#imagePreview'); // Add an image preview container if not present
        imagePreview.src = recipe.image;
        imagePreview.style.display = 'block';
    }

    // If using MDB Select, reinitialize the selects
